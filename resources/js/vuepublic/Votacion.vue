<template>
          ...
          <div class="card-body">
            <form>
              <div class="col-xxl">
                <div class="card mb-4" v-for="(grupo, index) in grupos" :key="index">
                  ...
                  <div class="card-body col-xxl">
                    <div class="col-sm-10" v-for="i in grupo.numCandidatos" :key="i">
                      <div class="row mb-3">
                        <label class="col-sm-2 col-form-label" :for="`candidato-${grupo.numero}-${i}`">
                          Candidato {{ i }}
                        </label>
                        <div class="col-sm-10">
                          <select
                            :id="`candidato-${grupo.numero}-${i}`"
                            class="form-select"
                            aria-label="Default select example"
                            v-model="votos[grupo.numero - 1][i - 1]"
                            @change="mostrarFotoSeleccionada(grupo.numero, i)"
                          >
                          ...
                          <div v-if="!votos[grupo.numero - 1][i - 1]" class="text-danger">
              ...
              <div class="row mb-3">
                ...
                <div class="col-sm-10">
                  <button type="button" class="btn btn-primary" @click="enviarVotacion" :disabled="yaVoto || !idUsuarioAutenticado">
                    Enviar votación
                  </button>
                  &nbsp;&nbsp;&nbsp;&nbsp;
                  <button type="button" class="btn btn-primary" @click="limpiarCampos" :disabled="yaVoto || !idUsuarioAutenticado">
                    Limpiar campos
                  </button>
                </div>
                <div class="card-header d-flex align-items-center justify-content-between">
  ...
  export default {
    ...
    created() {
      ...
      if (ronda === "1") {
        axios
          .get("/obtenerSegundaFechaConcurso")
          .then((response) => {
    ...
    methods: {
      verificarVotoUsuarioEnRonda(ronda) {
        axios
          .get(`/verificarVotoUsuarioActual/${ronda}`)
          .then((response) => {
      ...
      obtenerOpcionesVotacion() {
        ...
        axios
          .get(`/obtenerOpcionesVotacion/${ronda}`)
          .then((response) => {
            try {
              let dataArray;
              ...
              this.grupos = dataArray.reduce((accumulator, opcion) => {
                const existingGroup = accumulator.find(
                  (group) => group.numero === opcion.id_grup
                );
                ...
                if (existingGroup) {
                  ...
                  existingGroup.numCandidatos = Math.max(
                    existingGroup.numCandidatos,
                    2
                  );
                } else {
                  accumulator.push({
      ...
      obtenerIdUsuarioAutenticado() {
        axios
          .get("/obtenerIdUsuarioAutenticado")
          .then((response) => {
